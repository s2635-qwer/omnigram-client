...
export default function App() {
  ...
  async function loadPosts() {
    try {
      ...
      setPosts([]);
      setLoadError("서버에 연결할 수 없어요. Supabase 설정 또는 네트워크를 확인해주세요.");
    }
  }
          ...
          <div className="og-body">
                  ...
                  <div key={p.id} className="og-card">
                    ...
                    <div className="og-card-actions">
                      <button className={"og-action" + (liked ? " liked" : "")} onClick={() => toggleLike(p.id)}>
                        {liked ? "❤️" : "🤍"} {pcat.likeLabel} ({p.likes?.length || 0})
                      </button>
                      <button className="og-action" onClick={() => setOpenComments(prev => ({ ...prev, [p.id]: !prev[p.id] }))}>
                        💬 {pcat.commentLabel} ({p.comments?.length || 0})
                      </button>
                    </div>
                    ...
                    {isCommentsOpen && (
                      <div className="og-comments">
                        {(p.comments || []).map((c, idx) => (
                          <div key={idx} className="og-comment">
                            <b>{c.author}</b> {c.text}
                          </div>
                        ))}
                        <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
                          ...
                          />
                          <button className="og-btn" style={{ padding: "6px 12px" }} onClick={() => addComment(p.id)}>게시</button>
                        </div>
                      </div>
                    )}
                  </div>
            ...
            <div className="og-side">
              {activeCat === "study" && (
                <div style={{ marginBottom: 20 }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 6, fontSize: 13 }}>
                    {studyLeaderboard.map(([user, mins], i) => (
                      <div key={user} style={{ display: "flex", justifyContent: "space-between" }}>
                        <span>{i + 1}. <b>{user}</b></span>
                        <span style={{ color: "var(--accent)" }}>{mins}분</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {activeCat === "travel" && (
                <div style={{ marginBottom: 20 }}>
                  ...
                  {travelStamps.length === 0 ? (
                    <div style={{ fontSize: 12.5, color: "var(--muted)" }}>아직 기록한 여행지가 없어요.</div>
                  ) : (
                    <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                      {travelStamps.map((country) => (
                        <Chip key={country} color="var(--accent)">📍 {country}</Chip>
                      ))}
                    </div>
                  )}
                </div>
              ...
              <div style={{ fontSize: 12, color: "var(--muted)", lineHeight: 1.5 }}>
                <p><b>OmniGram Passport v2</b></p>
                <p>Supabase 실시간 데이터베이스 연동 프로토타입</p>
              </div>
            </div>
          </div>
        </div>
